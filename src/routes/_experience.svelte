<section>
  <h2>Experience</h2>

  <div class="column">
    <h3>
      <a href="https://squareup.com" rel="external" target="_blank">Square, Inc.</a>
    </h3>
    <p>Senior Frontend Engineer</p>
    <p class="date">Apr 2014 – Present</p>
    <p>Frontend engineering lead on:</p>
    <ul>
      <li>Shared navigation library featured on global marketing website and various subsites</li>
      <li>
        Design system framework powering marketing website, support center, hardware e-commerce
        store, and other public-facing web properties
      </li>
      <li>Integration of marketing website with 3rd-party headless content management system</li>
      <li>Hardware store build on open source e-commerce platform</li>
    </ul>
  </div>

  <div class="column">
    <h3>
      <a href="https://www.zappos.com" rel="external" target="_blank">Zappos Labs</a>
    </h3>
    <p>Senior Developer</p>
    <p class="date">May 2011 – Apr 2014</p>
    <p>Lead UX design and UI development on various e-commerce experiments</p>

    <h3>
      <a href="https://nickvincent.carbonmade.com" rel="external" target="_blank">Self-Employed</a>
    </h3>
    <p>Freelance Client Work</p>
    <p class="date">Apr 2010 – May 2011</p>

    <h3>
      <a href="https://www.rrpartners.com" rel="external" target="_blank">R&R Partners</a>
    </h3>
    <p>Web Developer</p>
    <p class="date">Apr 2009 – Apr 2010</p>

    <h3><a href="http://eatdrink.com" rel="external" target="_blank">Eatdrink</a></h3>
    <p>Web / Interactive Lead</p>
    <p class="date">Mar 2006 - Apr 2009</p>
    <p>Web Designer / Developer</p>
    <p class="date">Sep 2003 - Mar 2006</p>
    <p>Web Intern</p>
    <p class="date">Mar 2003 - Sep 2003</p>
  </div>
</section>

<style>
  @media print, (min-width: 50rem) {
    section {
      display: grid;
      grid-template-columns: 1fr 1fr;
      grid-template-rows: max-content;
      column-gap: var(--grid-margin);
      grid-column: span 2;
      grid-row: span 2;
    }

    h2 {
      grid-column: span 2;
    }
  }

  li {
    margin: 0 0 1rem;
  }
</style>
